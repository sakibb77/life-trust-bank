...
const accounts = [
  {
    ...
    movementsDates: [
      ...
      "2022-09-27T23:36:17.929Z",
      "2022-09-28T12:51:31.398Z",
...
/////////////////////////////////////////////////////////////
// set data and get data form local storage
/////////////////////////////////////////////////////////////

let allAccountData = accounts;
...
createUsernames(allAccountData);
...
function formateMoveDate(date, locale) {
  const calculationDays = (date1, date2) =>
    Math.round(Math.abs(date1 - date2) / (24 * 60 * 60 * 1000));

  const daypassed = calculationDays(new Date(), date);
...
btnLogin.addEventListener("click", function (e) {
  e.preventDefault();
  currentAccount = allAccountData.find(
    (account) => inputLoginUsername.value === account.username
  );
  if (inputLoginPassword.value === currentAccount?.password) {
    setTimeout(function () {
...
function displaySummary(account) {
  ...
  const income = account.movements
    .filter((move) => move > 0)
    .reduce((acc, diposit) => acc + diposit, 0);
...
btnTransfer.addEventListener("click", function (e) {
  e.preventDefault();
  const receiverAccount = allAccountData.find(
...
btnLoan.addEventListener("click", function (e) {
  ...
  if (
    ...
    setTimeout(() => {
      labelWelcome.textContent = "load not successful!";
    }, 3000);
    //set logout timer
    if (timer) clearInterval(timer);
    timer = logOut();
